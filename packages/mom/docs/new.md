...
```typescript
import { type AgentSessionEvent } from "@havasik/hotpi";
...
import type { MomCustomTool, ToolAPI } from "@havasik/mom";
...
Tools are loaded via jiti. They can import any 3rd party dependencies (install in the tool directory). Imports of `@havasik/ai` and `@havasik/mom` are aliased to the running mom bundle.
