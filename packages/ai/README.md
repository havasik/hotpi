...
```bash
npm install @havasik/ai
```

TypeBox exports are re-exported from `@havasik/ai`: `Type`, `Static`, and `TSchema`.

## Quick Start

```typescript
import { Type, getModel, stream, complete, Context, Tool, StringEnum } from '@havasik/ai';
...
```typescript
import { Type, Tool, StringEnum } from '@havasik/ai';
...
```typescript
import { stream, validateToolCall, Tool } from '@havasik/ai';
...
```typescript
import { readFileSync } from 'fs';
import { getModel, complete } from '@havasik/ai';
...
```typescript
import { getModel, streamSimple, completeSimple } from '@havasik/ai';
...
```typescript
import { getModel, stream } from '@havasik/ai';
...
```typescript
import { getProviders, getModels, getModel } from '@havasik/ai';
...
```typescript
import { Model, stream } from '@havasik/ai';
...
```typescript
import { streamAnthropic, type AnthropicOptions } from '@havasik/ai';
...
```typescript
import { getModel, complete, Context } from '@havasik/ai';
...
```typescript
import { Context, getModel, complete } from '@havasik/ai';
...
- OAuth login flows are not supported in browser environments. Use the `@havasik/ai/oauth` entry point in Node.js.
...
```typescript
import { getEnvApiKey } from '@havasik/ai';
...
```bash
npx @havasik/ai login              # interactive provider selection
npx @havasik/ai login anthropic    # login to specific provider
npx @havasik/ai list               # list available providers
```
...
The library provides login and token refresh functions via the `@havasik/ai/oauth` entry point. Credential storage is the caller's responsibility.
...
} from '@havasik/ai/oauth';
```

### Login Flow Example

```typescript
import { loginGitHubCopilot } from '@havasik/ai/oauth';
...
```typescript
import { getModel, complete } from '@havasik/ai';
import { getOAuthApiKey } from '@havasik/ai/oauth';
...
- Add any root-level `export type` re-exports in `src/index.ts` that should remain available from `@havasik/ai`
